                ...
                if (true) {
                    ...
                    if (preg_match("/^[a-zA-Z- 0-9.]+$/", $ec_name)) {
                        if (strlen($ec_name) <= 100) {
                            ...
                            $isSuccess = $GameMonetizeConnect->query("UPDATE ".TAGS." SET url='{$seo_name}',name='{$ec_name}',footer_description='{$ec_footer_description}', is_last_rewrite={$is_last_rewrite} WHERE id='{$ec_id}'");
                            if ($isSuccess) {
                                $data = array(
                                    'status' => 200,
